...
// import { Camera, CheckCircle, LoaderCircle, X } from 'lucide-react';
...
//           });
//           if (b.images || b.image) setFile(b.images ||b.image);
//         }
//       })
//       .catch((err) => {
//         console.error('Error fetching banner:', err);
//         // don't throw, show friendly alert
...
//     setBannerForm((p) => ({ ...p, [name]: type === 'checkbox' ? checked : value }));
//   };
...
//     if (!validTypes.includes(selected.type)) return alert('Invalid image type');
//     if (selected.size > 5 * 1024 * 1024) return alert('Max 5MB');
//     setFile(selected);
//   };
...
//     e.preventDefault();
//     if (!bannerForm.altText.trim()) return alert('Alt Text required');
...
//       fd.append('altText', bannerForm.altText.trim());
//       fd.append('linkUrl', bannerForm.linkUrl.trim());
//       fd.append('position', bannerForm.position);
//       fd.append('isActive', bannerForm.isActive.toString());
...
//       if (json.success) {
//         alert(bannerId ? 'Banner updated' : 'Banner created');
...
//           <h1 className="text-3xl font-bold text-gray-900 mb-2">{bannerId ? 'Edit Banner' : 'Add New Banner'}</h1>
//           <p className="text-gray-600">{bannerId ? 'Update banner details and image' : 'Upload banner image and configure settings'}</p>
//         </div>
...
//             <h2 className="text-xl font-semibold text-gray-900 border-b pb-2">Banner Details</h2>
//             <div className="grid grid-cols-1 gap-6">
//               <div className="space-y-2">
//                 <label className="text-sm font-medium text-gray-700">Alt Text for Banner *</label>
//                 <input name="altText" value={bannerForm.altText} onChange={handleChange} required className="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl" />
//               </div>

//               <div className="space-y-2">
//                 <label className="text-sm font-medium text-gray-700">Link URL (Optional)</label>
//                 <input type="url" name="linkUrl" value={bannerForm.linkUrl} onChange={handleChange} placeholder="https://example.com" className="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl" />
//               </div>

//               <div className="space-y-2">
//                 <label className="text-sm font-medium text-gray-700">Banner Position *</label>
//                 <select name="position" value={bannerForm.position} onChange={handleChange} className="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl">
...
//                 <input type="checkbox" name="isActive" checked={bannerForm.isActive} onChange={handleChange} className="w-4 h-4" />
//                 <label className="text-sm font-medium text-gray-700">Activate this banner immediately</label>
//               </div>
//             </div>
//           </div>

//           <div className="space-y-6">
//             <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-3 border-b pb-2"><Camera className="w-6 h-6 text-green-600" />Banner Image {!bannerId && '*'}</h2>

//             <div className="space-y-4">
//               <div className="flex flex-wrap gap-4">
//                 {file && (
//                   <div className="relative group">
//                     <div className="w-24 h-24 rounded-2xl border-2 border-green-200 overflow-hidden shadow-lg">
//                       <img src={getImageSrc()} alt="preview" className="w-full h-full object-cover" />
//                     </div>
//                     <button type="button" onClick={removeImage} className="absolute -top-2 -right-2 bg-red-500 text-white w-6 h-6 rounded-full">
//                       <X className="w-3 h-3" />
//                     </button>
//                   </div>
//                 )}
...
//                 {(!file || bannerId) && (
//                   <label className="w-24 h-24 border-3 border-dashed border-green-300 rounded-2xl flex flex-col items-center justify-center gap-2 cursor-pointer">
//                     <Camera className="w-8 h-8 text-green-500" />
//                     <span className="text-xs font-medium text-green-600">{bannerId && file ? 'Change' : 'Upload'}</span>
//                     <input type="file" onChange={handleFileChange} className="hidden" accept="image/jpeg,image/jpg,image/png,image/webp" />
//                   </label>
//                 )}
//               </div>

//               <p className="text-sm text-gray-500">{file ? 'Image selected' : 'No image selected'} • Supported formats: JPG, PNG, WebP • Max 5MB</p>
//             </div>
//           </div>

//           <div className="pt-6">
//             <button type="submit" disabled={loading || (!file && !bannerId) || !bannerForm.altText.trim()} className="w-full py-4 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold text-lg rounded-2xl">
//               {loading ? <span>Saving...</span> : <span>{bannerId ? 'Update Banner' : 'Add Banner'}</span>}
//             </button>
//           </div>
//         </form>
//       </div>
